...
class NicfpsExposureActor(InstExposure.InstExposure):
    def __init__(self, **argv):
        InstExposure.InstExposure.__init__(self, 'nicfpsExpose', debug=1)
        
        self.instName = 'nicfps'

        self.helpText = ("nicfpsExpose COMMAND [ARGS]",
                         "   COMMAND is one of:",
                         "     help         - you got it!",
    ...
    def _parse(self, cmd):
        ...
        elif command in ('stop', 'abort', 'zap'):
            if exp == None:
...
def main(name, eHandler=None, debug=0, test=False):
    if eHandler == None:
        eHandler = NicfpsExposureActor(debug=1)
    eHandler.start()
