...
class SpicamExposureActor(InstExposure.InstExposure):
    def __init__(self, **argv):
        InstExposure.InstExposure.__init__(self, 'spicamExpose', debug=1)
        
        self.instName = 'spicam'

        self.helpText = ("spicamExpose COMMAND [ARGS]",
                         "   COMMAND is one of:",
                         "     help         - you got it!",
    ...
    def _parse(self, cmd):
        ...
        if self.debug >= 0:
            CPL.log("SpicamExposureHandler", "new command: %s" % (cmd.raw_cmd))
